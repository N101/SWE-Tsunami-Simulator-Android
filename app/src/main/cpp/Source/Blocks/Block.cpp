...
//  // block = new WaveAccumulationBlock(nx, ny, dx, dy);
  block = new Blocks::DimensionalSplitting(nx, ny, dx, dy);
//  // block = new DimensionalSplitting(nx, ny, dx, dy);
//#elif defined(WITH_SOLVER_RUSANOV)
...
BoundaryType* Blocks::Block::getBoundaryType() {
  synchBoundaryTypeBeforeRead();
  return boundary_;
}
...
void Blocks::Block::synchBoundaryTypeBeforeRead() {}
